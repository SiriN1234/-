...
who_disease = read_xlsx("C:/Users/SiriN/Documents/R/temp/who_disease.xlsx")
...
ggplot(amr_pertussis, aes(x = cases)) + 
  geom_histogram(fill = "peachpuff2", 
                 color = "white") + 
  theme_minimal()
...
ggplot(gender_data, 
  ...
  geom_errorbar(aes(ymin = mean - se, 
                ...
                width = .1, 
                position= pd) +   #에러바 그림
  scale_y_continuous(label = scales::dollar) +   #y축에 달러 넣기
