...
# 수강생이 원하는 변수를 추출하세요. 
counties %>% 
  select(men, women, drive, carpool, transit, walk)
...
counties_selected %>% 
  arrange(desc(public_work)) # arrange(desc())은 내림차순

# self_employed 변수를 기준으로 오름차순으로 정렬합니다. 
# 수강생이 직접입력합니다. (주석을 풉니다. )
counties_selected %>% 
  arrange(self_employed) # 오름차순은 그냥 arrange()를 쓰면 됨
# 
...
write.csv(x = counties_selected, "결과물.csv")
...
counties %>%
  select(state, population, private_work, self_employed) %>%
  group_by(state) %>%
  summarise(min_pop = min(population),
            max_pop = max(population),
            avg_pop = mean(population),
            min_pri = min(private_work))
...
#데이터 %>%
#  select() # 변수 추출
#  mutate() # c = a column + b column 새로운 파생 변수
#  group_by(# 주로 문자 데이터)
#  summarise(# 주로 수치형 칼럼) -> 결과저장
